...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/eight_ball/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/eight_ball/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/eight_ball/*"
            )
            ...
            #    "--include=src/saltext/eight_ball/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/eight_ball/config/schemas",
    )
